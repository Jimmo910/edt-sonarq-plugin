...
- **Настройка применяемых проверок** — страница «Проверки BSL» с группировкой проверок по
  категориям (дубли валидатора EDT, требующие настройки, неуместные, прочие) и кнопкой
...
- Проверки показаны деревом с группировкой по категориям (колонки **Ключ** и **Название**):
  **«Дубли валидатора EDT»** — проверки, дублирующие встроенную проверку EDT (тултип строки
  называет id этой проверки EDT); **«Требуют настройки под проект»** — проверки по метрикам и
  порогам (сложность, размеры, магические числа и т. п.); **«Неуместны в EDT»** — спорные или
  конфликтующие с EDT проверки; **«Прочие»** — всё остальное. У каждой категории — свой флажок
  группы (переключает разом все проверки внутри неё; показывает промежуточное состояние, если
  часть проверок включена, а часть выключена) и собственный счётчик **«Выключено N из M»**.
  Включённая проверка (флажок установлен) применяется, выключенная — игнорируется. По умолчанию
  включены все проверки.
- Кнопка **«Применить рекомендованный профиль»** одним нажатием выключает рекомендованный набор
  — дубли валидатора EDT + проверки, требующие настройки под проект, + неуместные в EDT
  (категорию «Прочие» кнопка не трогает). Это осознанное действие пользователя (opt-in): пока
  кнопка не нажата, ничего не меняется — включены все проверки, как и раньше.
- Разбивка проверок по категориям — это первый, заведомо неполный срез, встроенный в плагин;
  сообщество может уточнять его через Pull Request (см.
...
- Разбивка проверок по категориям («Проверки BSL») — заведомо неполный первый срез, встроенный
  в плагин; уточняется сообществом через Pull Request (см.
  [issue #3](https://github.com/Jimmo910/edt-sonarq-plugin/issues/3)).
